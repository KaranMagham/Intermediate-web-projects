...
const ManageTransactions = () => {
    ...
    const [savedTransactions, setsavedTransactions] = useState(() => {
        const stored = localStorage.getItem("savedTransactions");
    ...
    const onSubmit = async (data) => {
        ...
        if (editIndex !== null) {
            setsavedTransactions(prev => {
                const updated = [...prev];
                updated.splice(editIndex, 1, newEntry);
                return updated;
            });
            setEditIndex(null);
        } else {
            setsavedTransactions(prev => [...prev, newEntry]);
        }
                    ...
                    <form onSubmit={handleSubmit(onSubmit)}
                        ...
                        <div className="w-full rounded-xl border border-yellow-600 p-4 sm:p-6 space-y-4">
                            ...
                            {savedTransactions.map((entry, index) => (
                                <div key={entry.id} className="p-4 rounded-lg border border-yellow-600 space-y-2">
